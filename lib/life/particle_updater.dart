...
class ParticleUpdater {
  ...
  late num rMin;
  num? rMax;
  late num forceFactor;
  late num heat;
  late num boxWidth;
  late num boxHeight;
  late num currentDeltaT;
  late num interactionRadiusSquared;
  late num oneMinusFrictionDt;

  void setValues(num rMin, num rMax, num forceFactor, num friction, num heat,
      num boxWidth, num boxHeight, num dt) {
    this.rMin = rMin;
    this.rMax = rMax;
    interactionRadiusSquared = rMax * rMax;
    this.forceFactor = forceFactor;
    this.heat = heat;
    this.boxWidth = boxWidth;
    this.boxHeight = boxHeight;
    currentDeltaT = dt;
    oneMinusFrictionDt = max(0, 1 - friction * currentDeltaT);
  }

  void updateWithRelevant(
      Particle particle, List<Particle> relevant, Matrix? matrix) {
    for (Particle particle2 in relevant) {
      if (particle != particle2) {
        ...
        if (d2 > 0 && d2 < interactionRadiusSquared) {
          ...
          num a = forceFactor *
              getForce(
                  d, rMin, rMax, matrix!.get(particle.type, particle2.type));
