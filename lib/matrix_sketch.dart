...
import 'package:particle_life_flutter/life/color_maker.dart';
...
class MatrixSketch extends PPainter {
  static const int minColorBorderSize = 10;
  Matrix? _matrix;

  // paint values
  late double sx;
  late double sy;
  late num leftBorderSize;
  late num topBorderSize;

  final SelectionCallback? _selectionCallback;
  bool _selectionActive;
  int? _iSelected;
  int? _jSelected;

  MatrixSketch(this._matrix,
      {bool active = false,
      int? i = 0,
      int? j = 0,
  ...
  void mousePressed() {
    ...
    if (i >= 0 && i < _matrix!.n && j >= 0 && j < _matrix!.n) {
  ...
  void _calcPaintValues() {
    ...
    sx = ((paintSize.width - minColorBorderSize) / n).floor().toDouble();
    sy = ((paintSize.height - minColorBorderSize) / n).floor().toDouble();
  ...
  void draw() {
    ...
    if (_selectionActive) {
      ...
      strokeWeight(2);
      noFill();
      rect(leftBorderSize + _jSelected! * (sx),
          topBorderSize + _iSelected! * (sy), sx, sy);
      pop();
    }
  }
}
